// Declare global variables provided by CDN scripts to TypeScript
declare const ExcelJS: any;
declare const dayjs: any;

// Extend the Window interface to include the dayjs plugin for TypeScript
declare global {
  interface Window {
...
export const convertDatesInXlsx = async (file: File): Promise<Blob> => {
  // Ensure the dayjs customParseFormat plugin is loaded
  if (typeof dayjs.extend !== 'function' || !window.dayjs_plugin_customParseFormat) {
    throw new Error('Day.js or its customParseFormat plugin is not loaded.');
  }
        ...
        row.eachCell({ includeEmpty: true }, (cell: any) => {
          ...
          // Case 3: Cell is a number, but might represent a date (Excel's date serial number)
            ...
            if (nf.includes('y') || nf.includes('m') || nf.includes('d')) {
                 ...
                 const jsDate = new Date((cell.value - 25569) * 86400 * 1000);
